...
class CIFAR100Loader:
    def __init__(self, data_dir='data'):
        """
        初始化CIFAR-100数据加载器，使用torchvision自动下载并加载数据集。
    ...
    def load_data(self):
        """
        使用 torchvision 加载 CIFAR-100 数据集
        :return: X, y 数据特征和标签
        """
        # 加载训练集和测试集
        train_dataset = CIFAR100(root=self.data_dir, train=True, download=True, transform=self.transform)
        test_dataset = CIFAR100(root=self.data_dir, train=False, download=True, transform=self.transform)
...
class MNISTLoader:
    def __init__(self, data_dir='data'):
        """
        初始化MNIST数据加载器，使用torchvision自动下载并加载数据集。
        :param data_dir: 数据集保存路径
        """
        self.data_dir = data_dir
        self.transform = transforms.Compose([
    ...
    def load_data(self):
        ...
        """
        # 加载训练集和测试集
        train_dataset = MNIST(root=self.data_dir, train=True, download=True, transform=self.transform)
        test_dataset = MNIST(root=self.data_dir, train=False, download=True, transform=self.transform)
        ...
        # 将X的每一个样本也变成(Height, Width, Channel)形式
